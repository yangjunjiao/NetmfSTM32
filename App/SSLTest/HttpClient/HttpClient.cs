...
namespace HttpClientSample
{
    ...
    public static class MyHttpClient
    {
        ...
        {
            Microsoft.SPOT.Hardware.Utility.SetLocalTime(new DateTime(2009, 4, 14));

           

            Thread.Sleep(1000);
            //activateDHCP();
            //setIp("192.168.0.200", "255.255.255.0", "192.168.0.1");
            Thread.Sleep(1000);
            printIP();
           ...
           /* // Root CA Certificate needed to validate HTTPS servers.
            ...
            while (true)
            {
                ...
                //PrintHttpData("http://autos.msn.com/default.aspx");
               // PrintHttpData("http://www.nytimes.com/");
        ...
        private static void printIP()
        {
            ...
            foreach (var allNetworkInterface in allNetworkInterfaces)
            {
                ...
                {
                    Debug.Print(allNetworkInterface.IPAddress);
                }
            }
        }

        private static void setIp(string ipAddress, string subNet, string gateway)
        {
            NetworkInterface[] allNetworkInterfaces = NetworkInterface.GetAllNetworkInterfaces();
            foreach (var allNetworkInterface in allNetworkInterfaces)
            {
                if (allNetworkInterface.NetworkInterfaceType == NetworkInterfaceType.Ethernet)
                {
                    allNetworkInterface.EnableStaticIP(ipAddress, subNet, gateway);
                }
            }
        }

        private static void activateDHCP()
        {
            NetworkInterface[] allNetworkInterfaces = NetworkInterface.GetAllNetworkInterfaces();
            foreach (var allNetworkInterface in allNetworkInterfaces)
            {
        ...
        public static void PrintHttpDataOld(string url)
        {
                ...
                else
                {
                    ...
                    while (true)
                    {
                        ...
                        catch (System.Net.Sockets.SocketException e)
                        {
                            ...
                            Debug.Print("ErrorCode: " + e.ErrorCode);
                            bytesRead = 0;
                        }
