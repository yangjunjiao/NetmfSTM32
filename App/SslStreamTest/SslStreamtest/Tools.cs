...
namespace Microsoft.SPOT.Platform.Tests
{
    class Tools
    {
        ...
        static public bool CertificateCompare(X509Certificate cert1, X509Certificate cert2)
        {
            ...
            {
                Debug.Print("One of the certificates is null");
                return false;
            }
        ...
        static public bool CertificateCompare(byte[] cert1Bytes, byte[] cert2Bytes)
        {
            ...
            {
                Debug.Print("One of the byte arrays is null");
                return false;
            }

            if( cert1Bytes.Length != cert2Bytes.Length )
            {
                Debug.Print("The certificates are not the same length");
                return false;
            }
            ...
            for( int i=0; i< cert1Bytes.Length; ++i)
            {
                ...
                {
                    Debug.Print("The certificates are not equal");
                    return false;
                }
            }
            return true;
