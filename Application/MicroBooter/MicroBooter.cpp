...
static int s_WriteBufferIndex = 0;
...
{
    INT32 eraseSize = s_MemoryStreamSrcEraseSize;
    int index = (address % eraseSize);

    ASSERT(length == 1);
    
    if(s_ReadBuffer == NULL)
    {
        s_ReadBuffer = (UINT8*)private_malloc(eraseSize);
        ...
        if(0 != index)
        {
            s_MemoryStreamSrc->Read( s_MemoryStreamSrcHandle, s_MemoryStreamSrcOffset - index, s_ReadBuffer, eraseSize );

            s_MemoryStreamSrcOffset += eraseSize - index;
        }
    }
    ...
    if(s_ReadBuffer != NULL)
    {
        if(0 == index)
        {
            if(eraseSize <= ((UINT32)&HeapEnd - (UINT32)&HeapBegin))
            {
                s_MemoryStreamSrc->Read( s_MemoryStreamSrcHandle, s_MemoryStreamSrcOffset, s_ReadBuffer, eraseSize );

                s_MemoryStreamSrcOffset += eraseSize;
            }
            else
            {
...
BOOL MicroBooter_Install(HAL_UPDATE_CONFIG updateCfg)
{
    ...
    for(int i=0; i<g_MicroBooter_UpdateStorageListCount; i++)
    {
