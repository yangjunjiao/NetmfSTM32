...
HRESULT Library_corlib_native_System_Number::FormatNative___STATIC__STRING__OBJECT__CHAR__I4( CLR_RT_StackFrame& stack )
{
    ...
    if(formatCh == 'X')
    {
        hal_snprintf( formatStr, ARRAYSIZE(formatStr), "%%0%dX" , precision ); // "%0[n]X"  (wher n is the number chars to write)

        switch(dt)
        {
            case DATATYPE_I1: 
                hal_snprintf( result, ARRAYSIZE(result), formatStr, value->NumericByRef().s1 ); 
                break;
            case DATATYPE_U1: 
                hal_snprintf( result, ARRAYSIZE(result), formatStr, value->NumericByRef().u1 ); 
                break;
            case DATATYPE_I2: 
                hal_snprintf( result, ARRAYSIZE(result), formatStr, value->NumericByRef().s2 ); 
                break;
            case DATATYPE_U2: 
                hal_snprintf( result, ARRAYSIZE(result), formatStr, value->NumericByRef().u2 ); 
                break;
            case DATATYPE_I4: 
                hal_snprintf( result, ARRAYSIZE(result), formatStr, value->NumericByRef().s4 ); 
                break;
            case DATATYPE_U4: 
                hal_snprintf( result, ARRAYSIZE(result), formatStr, value->NumericByRef().u4 ); 
                break;
            ...
            case DATATYPE_I8: 
                {
                    CLR_INT64 val = (CLR_INT64_TEMP_CAST)value->NumericByRef().s8;
                    hal_snprintf( result, ARRAYSIZE(result), formatStr, val); 
                }
                break;
            case DATATYPE_U8:
                {
                    CLR_UINT64 val = (CLR_UINT64_TEMP_CAST)value->NumericByRef().u8;
                    hal_snprintf( result, ARRAYSIZE(result), formatStr, val ); 
                }
                break;
#endif
