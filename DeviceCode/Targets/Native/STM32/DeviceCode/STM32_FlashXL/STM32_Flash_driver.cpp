...
#include "..\stm32f10x.h"
	...
	static const STM32_Flash_Driver::CHIP_WORD *  BANK2_START     = (STM32_Flash_Driver::CHIP_WORD *)0x08080000;      // Bank1 Size = 512kB, offset 0x0800 0000
...
BOOL __section(SectionForFlashOperations)STM32_Flash_Driver::WriteBank1(
	CHIP_WORD* ChipAddress,
    CHIP_WORD* EndAddress,
    CHIP_WORD *pBuf)
{
    ...
    while(ChipAddress < EndAddress) {
        if (*ChipAddress != *pBuf) {
            ...
            *ChipAddress = *pBuf;
            ...
            // check
            if (*ChipAddress != *pBuf) {
                debug_printf( "Flash_WriteToSector Bank1 failure @ 0x%08x, wrote 0x%08x, read 0x%08x\r\n", (UINT32)ChipAddress, *pBuf, *ChipAddress );
                return FALSE;
            }
        }
        ChipAddress++;
...
BOOL __section(SectionForFlashOperations)STM32_Flash_Driver::WriteBank2(
	CHIP_WORD* ChipAddress,
    CHIP_WORD* EndAddress,
    CHIP_WORD *pBuf)
{
	if (FLASH->CR2 & FLASH_CR_LOCK) { // unlock
        FLASH->KEYR2 = STM32_FLASH_KEY1;
    ...
    while(ChipAddress < EndAddress) {
        if (*ChipAddress != *pBuf) {
            ...
            *ChipAddress = *pBuf;
            ...
            // check
            if (*ChipAddress != *pBuf) {
                debug_printf( "Flash_WriteToSector Bank2 failure @ 0x%08x, wrote 0x%08x, read 0x%08x\r\n", (UINT32)ChipAddress, *pBuf, *ChipAddress );
                return FALSE;
            }
        }
        ChipAddress++;
...
BOOL __section(SectionForFlashOperations)STM32_Flash_Driver::EraseBlockBank1(ByteAddress Sector )
{
...
BOOL __section(SectionForFlashOperations)STM32_Flash_Driver::EraseBlockBank2(ByteAddress Sector )
{
...
BOOL __section(SectionForFlashOperations)STM32_Flash_Driver::EraseBlock( void* context, ByteAddress Sector )
{
    ...
    RCC->CR |= RCC_CR_HSION;
    while(!(RCC->CR & RCC_CR_HSIRDY));
    if(Sector < BANK2_START)
	{
		EraseBlockBank1(Sector);
	}
	else
	{
		EraseBlockBank2(Sector);
	}
