...
/*********************************************************************************************
 * @file    STM32F2_ETH_driver.cpp
 * @brief   STM32F2 ethernet driver. 
...
// PHY
#define PHY_ADDRESS                     0x01U           // PHY address

// MACMIIAR
#define MACMIIAR_PA_POSITION            11U             // PHY address bit position
#define MACMIIAR_MR_POSITION            6U              // MII register bit position
#define MII_BUSY_TIMEOUT                0x00000FFFU     // PHY read and write register timeout

// Timeouts
#define MAC_RESET_TIMEOUT               0x00000FFFU     // MAC reset timeout
#define FLUSH_TX_FIFO_TIMEOUT           0x00000FFFU     // Flush TX FIFO timeout

// Enable normal DMA interrupts
#define INT_ENABLE_NIS        1                         // Normal interrupt summary
#define INT_ENABLE_TI         0                         // Transmit
...
void eth_enableTxRx()
{
    ...
    // Start MAC and DMA transmission
    flushTxFifo();
    ETH->MACCR |= ETH_MACCR_TE;
    ETH->DMAOMR |= ETH_DMAOMR_ST;

    // Start DMA and MAC reception
...
{
    // Stop MAC and DMA reception
    ETH->MACCR &= ~ETH_MACCR_RE;
    ETH->DMAOMR &= ~ETH_DMAOMR_SR;
    
    // Stop DMA and MAC transmission
    ETH->DMAOMR &= ~ETH_DMAOMR_ST;
    ETH->MACCR &= ~ETH_MACCR_TE;
