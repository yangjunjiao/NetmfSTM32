...
/*********************************************************************************************
 * @file    STM32F2_ETH_lwip.cpp
 * @brief   Interface between the LWIP module and the STM32F2 ethernet driver. 
...
#include "STM32F2_ETH_rx_desc.h"
#include "STM32F2_ETH_tx_desc.h"
 ...
 */
BOOL STM32F2_ETH_LWIP_open(Netif_t *const pNetif)
{ 
    // Initialize PHY
    eth_initPhy();
 ...
 *  @arg FALSE otherwise.
 */
void STM32F2_ETH_LWIP_close(const BOOL disableClocks)
{
    // Set flag as closed
    s_isDriverOpened = FALSE;
 ...
 * @param pNetif the lwip network interface.
 */
void STM32F2_ETH_LWIP_recv(Netif_t *const pNetif)
{
    uint32_t iRxDesc = 0;
 ...
 */
err_t STM32F2_ETH_LWIP_xmit(Netif_t *const pNetif, Pbuf_t *const pbuf)
{
 ...
 * @param pNetif the lwip network interface.
 */
void STM32F2_ETH_LWIP_interrupt(Netif_t *const pNetif)
{
    // Disable preempting
    GLOBAL_LOCK(irq);
