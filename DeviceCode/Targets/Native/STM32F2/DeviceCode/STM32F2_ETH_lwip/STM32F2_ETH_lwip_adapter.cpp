...
#include "..\STM32F2_Power\STM32F2_Power_functions.h"

#include "STM32F2_ETH_lwip_adapter.h"
#include "STM32F2_ETH_lwip.h"
#include "STM32F2_ETH_driver.h"

#if defined(ADS_LINKER_BUG__NOT_ALL_UNUSED_VARIABLES_ARE_REMOVED)
#pragma arm section zidata = "g_STM32F2_ETH_lwip_adapter"
#endif

static struct netif          g_STM32F2_ETH_NetIF;
...
void STM32F2_ETH__status_callback(struct netif *netif)
{
...
err_t STM32F2_ETH_ethhw_init(struct netif *myNetIf) 
{ 
    /* Initialize netif */
    myNetIf->name[0] = 'e';
    myNetIf->name[1] = '0';
    ...
    myNetIf->linkoutput = STM32F2_ETH_LWIP_xmit;
    myNetIf->status_callback = STM32F2_ETH__status_callback;
    myNetIf->flags = NETIF_FLAG_BROADCAST | NETIF_FLAG_ETHARP | NETIF_FLAG_LINK_UP;
  
    /* Open ethernet driver */
    LwipNetworkStatus = STM32F2_ETH_LWIP_open(myNetIf);
...
{
    InterruptTaskContinuation.InitializeCallback( (HAL_CALLBACK_FPN)STM32F2_ETH_LWIP_recv, (void*)pNetIf );
...
void EthernetWakeUp()
{
    CPU_INTC_ActivateInterrupt(ETH_IRQn, (HAL_CALLBACK_FPN)STM32F2_ETH_LWIP_interrupt, &g_STM32F2_ETH_NetIF);
    InitContinuations(&g_STM32F2_ETH_NetIF);
}

void EthernetDeepSleep()
{
    netif_set_down(&g_STM32F2_ETH_NetIF);
    STM32F2_ETH_LWIP_close(FALSE);
    LwipNetworkStatus = 0;
    eth_powerUpPhy(FALSE);
...
int STM32F2_ETH_LWIP_Driver::Open(int index)
{
    ...
    STM32F2_SetPowerHandlers(EthernetDeepSleep, EthernetWakeUp);
    
    /* Apply network configuration */
    iface = &g_NetworkConfig.NetworkInterfaces[index];
    
    len = g_STM32F2_ETH_NetIF.hwaddr_len;
    if (len == 0 || iface->macAddressLen < len)
    {
        len = iface->macAddressLen;
        g_STM32F2_ETH_NetIF.hwaddr_len = len;
    }
    memcpy(g_STM32F2_ETH_NetIF.hwaddr, iface->macAddressBuffer, len);
    
    ipaddr.addr = iface->ipaddr;
    gateway.addr = iface->gateway;
    subnetmask.addr = iface->subnetmask;
      
    pNetIF = netif_add( &g_STM32F2_ETH_NetIF, &ipaddr, &subnetmask, &gateway, NULL, STM32F2_ETH_ethhw_init, ethernet_input );
    ...
    return g_STM32F2_ETH_NetIF.num;    
}

// -- //

BOOL STM32F2_ETH_LWIP_Driver::Close(void)
{
    CPU_INTC_DeactivateInterrupt(ETH_IRQn);
    ...
    netif_set_down( &g_STM32F2_ETH_NetIF );
    netif_remove( &g_STM32F2_ETH_NetIF );

    STM32F2_ETH_LWIP_close(TRUE);
  
    memset( &g_STM32F2_ETH_NetIF, 0, sizeof g_STM32F2_ETH_NetIF );

    return TRUE;
}

BOOL STM32F2_ETH_LWIP_Driver::Bind(void)
{
    return TRUE;
}
