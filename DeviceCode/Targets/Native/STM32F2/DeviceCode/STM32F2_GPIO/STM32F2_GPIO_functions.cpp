...
#define STM32F2_Gpio_MaxPins (TOTAL_GPIO_PORT * 16)
#define STM32F2_Gpio_MaxInt 16
...
static GPIO_INTERRUPT_SERVICE_ROUTINE g_ISR[STM32F2_Gpio_MaxInt]; // interrupt handlers
static void* g_ISR_Param[STM32F2_Gpio_MaxInt]; // interrupt handler parameters
...
void STM32F2_GPIO_ISR (int num)  // 0 <= num <= 15
{
    INTERRUPT_START
...
void STM32F2_GPIO_Interrupt0 (void* param) // EXTI0
{
    EXTI->PR = 1 << 0;   // reset pending bit
    STM32F2_GPIO_ISR(0);
}

void STM32F2_GPIO_Interrupt1 (void* param) // EXTI1
{
    EXTI->PR = 1 << 1;   // reset pending bit
    STM32F2_GPIO_ISR(1);
}

void STM32F2_GPIO_Interrupt2 (void* param) // EXTI2
{
    EXTI->PR = 1 << 2;   // reset pending bit
    STM32F2_GPIO_ISR(2);
}

void STM32F2_GPIO_Interrupt3 (void* param) // EXTI3
{
    EXTI->PR = 1 << 3;   // reset pending bit
    STM32F2_GPIO_ISR(3);
}

void STM32F2_GPIO_Interrupt4 (void* param) // EXTI4
{
    EXTI->PR = 1 << 4;   // reset pending bit
    STM32F2_GPIO_ISR(4);
}

void STM32F2_GPIO_Interrupt5 (void* param) // EXTI5 - EXTI9
{
    ...
    int num = 5; pending >>= 5;
    do {
        if (pending & 1) STM32F2_GPIO_ISR(num);
        num++; pending >>= 1;
    } while (pending);
}

void STM32F2_GPIO_Interrupt10 (void* param) // EXTI10 - EXTI15
{
...
BOOL STM32F2_GPIO_Set_Interrupt( UINT32 pin, GPIO_INTERRUPT_SERVICE_ROUTINE ISR, void* ISR_Param, GPIO_INT_EDGE mode)
{
    UINT32 num = pin & 0x0F;
    UINT32 bit = 1 << num;
...
void STM32F2_GPIO_Pin_Config( GPIO_PIN pin, UINT32 mode, GPIO_RESISTOR resistor, UINT32 alternate )
{
...
BOOL CPU_GPIO_Initialize()
{
    ...
    CPU_INTC_ActivateInterrupt(EXTI0_IRQn, STM32F2_GPIO_Interrupt0, 0);
    CPU_INTC_ActivateInterrupt(EXTI1_IRQn, STM32F2_GPIO_Interrupt1, 0);
    CPU_INTC_ActivateInterrupt(EXTI2_IRQn, STM32F2_GPIO_Interrupt2, 0);
    CPU_INTC_ActivateInterrupt(EXTI3_IRQn, STM32F2_GPIO_Interrupt3, 0);
    CPU_INTC_ActivateInterrupt(EXTI4_IRQn, STM32F2_GPIO_Interrupt4, 0);
    CPU_INTC_ActivateInterrupt(EXTI9_5_IRQn, STM32F2_GPIO_Interrupt5, 0);
    CPU_INTC_ActivateInterrupt(EXTI15_10_IRQn, STM32F2_GPIO_Interrupt10, 0);
...
void CPU_GPIO_DisablePin( GPIO_PIN pin, GPIO_RESISTOR resistor, UINT32 output, GPIO_ALT_MODE alternate )
{
    ...
    if (pin < STM32F2_Gpio_MaxPins) {
        ...
        STM32F2_GPIO_Pin_Config(pin, mode, resistor, (UINT32)alternate);
        STM32F2_GPIO_Set_Interrupt(pin, NULL, 0, GPIO_INT_NONE); // disable interrupt
    }
}
...
void CPU_GPIO_EnableOutputPin( GPIO_PIN pin, BOOL initialState )
{
    ...
    if (pin < STM32F2_Gpio_MaxPins) {
        ...
        STM32F2_GPIO_Pin_Config(pin, 1, RESISTOR_DISABLED, 0); // general purpose output
        STM32F2_GPIO_Set_Interrupt(pin, NULL, 0, GPIO_INT_NONE); // disable interrupt
    }
}
...
{
    NATIVE_PROFILE_HAL_PROCESSOR_GPIO();
    if (pin >= STM32F2_Gpio_MaxPins) return FALSE;
    STM32F2_GPIO_Pin_Config(pin, 0, resistor, 0); // input
    
    if (edge > GPIO_INT_EDGE_BOTH) return FALSE;
    return STM32F2_GPIO_Set_Interrupt(pin, ISR, ISR_Param, edge);
}
...
{
    NATIVE_PROFILE_HAL_PROCESSOR_GPIO();
    return STM32F2_Gpio_MaxPins;
}

void CPU_GPIO_GetPinsMap( UINT8* pins, size_t size )
{
    NATIVE_PROFILE_HAL_PROCESSOR_GPIO();
    for (int i = 0; i < size && i < STM32F2_Gpio_MaxPins; i++) {
