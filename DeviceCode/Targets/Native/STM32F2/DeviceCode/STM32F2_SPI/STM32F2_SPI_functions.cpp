...
// Pins
static const BYTE g_STM32F2_Spi_Sclk_Pins[] = STM32F2_SPI_SCLK_PINS;
static const BYTE g_STM32F2_Spi_Miso_Pins[] = STM32F2_SPI_MISO_PINS;
static const BYTE g_STM32F2_Spi_Mosi_Pins[] = STM32F2_SPI_MOSI_PINS;
#define STM32F2_SPI_MODS ARRAYSIZE_CONST_EXPR(g_STM32F2_Spi_Sclk_Pins) // number of modules
...
{
    NATIVE_PROFILE_HAL_PROCESSOR_SPI();
    if (Configuration.SPI_mod >= STM32F2_SPI_MODS) return FALSE;
...
void CPU_SPI_GetPins( UINT32 spi_mod, GPIO_PIN& msk, GPIO_PIN& miso, GPIO_PIN& mosi )
{
    ...
    if (spi_mod >= STM32F2_SPI_MODS) return;
    msk  = g_STM32F2_Spi_Sclk_Pins[spi_mod];
    miso = g_STM32F2_Spi_Miso_Pins[spi_mod];
    mosi = g_STM32F2_Spi_Mosi_Pins[spi_mod];
}
