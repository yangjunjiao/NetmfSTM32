...
// IO addresses
static const ptr_USART_TypeDef g_STM32F2_Uart_Ports[] = {USART1, USART2, USART3, UART4, UART5, USART6}; 

// Pins
static const BYTE g_STM32F2_Uart_RxD_Pins[] = STM32F2_UART_RXD_PINS;
static const BYTE g_STM32F2_Uart_TxD_Pins[] = STM32F2_UART_TXD_PINS;
static const BYTE g_STM32F2_Uart_CTS_Pins[] = STM32F2_UART_CTS_PINS;
static const BYTE g_STM32F2_Uart_RTS_Pins[] = STM32F2_UART_RTS_PINS;


void STM32F2_USART_Handle_RX_IRQ (int ComPortNum, USART_TypeDef* uart)
{
    INTERRUPT_START;
...
void STM32F2_USART_Handle_TX_IRQ (int ComPortNum, USART_TypeDef* uart)
{
    INTERRUPT_START;
...
void STM32F2_USART_Interrupt0 (void* param)
{
    UINT16 sr = USART1->SR;
    if (sr & USART_SR_RXNE) STM32F2_USART_Handle_RX_IRQ(0, USART1);
    if (sr & USART_SR_TXE)  STM32F2_USART_Handle_TX_IRQ(0, USART1);
}

void STM32F2_USART_Interrupt1 (void* param)
{
    UINT16 sr = USART2->SR;
    if (sr & USART_SR_RXNE) STM32F2_USART_Handle_RX_IRQ(1, USART2);
    if (sr & USART_SR_TXE)  STM32F2_USART_Handle_TX_IRQ(1, USART2);
}

void STM32F2_USART_Interrupt2 (void* param)
{
    UINT16 sr = USART3->SR;
    if (sr & USART_SR_RXNE) STM32F2_USART_Handle_RX_IRQ(2, USART3);
    if (sr & USART_SR_TXE)  STM32F2_USART_Handle_TX_IRQ(2, USART3);
}

void STM32F2_USART_Interrupt3 (void* param)
{
    UINT16 sr = UART4->SR;
    if (sr & USART_SR_RXNE) STM32F2_USART_Handle_RX_IRQ(3, UART4);
    if (sr & USART_SR_TXE)  STM32F2_USART_Handle_TX_IRQ(3, UART4);
}

void STM32F2_USART_Interrupt4 (void* param)
{
    UINT16 sr = UART5->SR;
    if (sr & USART_SR_RXNE) STM32F2_USART_Handle_RX_IRQ(4, UART5);
    if (sr & USART_SR_TXE)  STM32F2_USART_Handle_TX_IRQ(4, UART5);
}

void STM32F2_USART_Interrupt5 (void* param)
{
    UINT16 sr = USART6->SR;
    if (sr & USART_SR_RXNE) STM32F2_USART_Handle_RX_IRQ(5, USART6);
    if (sr & USART_SR_TXE)  STM32F2_USART_Handle_TX_IRQ(5, USART6);
}
...
BOOL CPU_USART_Initialize( int ComPortNum, int BaudRate, int Parity, int DataBits, int StopBits, int FlowValue )
{
    ...
    ptr_USART_TypeDef uart = g_STM32F2_Uart_Ports[ComPortNum];
    UINT32 clk;
    ...
    switch (ComPortNum) {
    case 0: CPU_INTC_ActivateInterrupt(USART1_IRQn, STM32F2_USART_Interrupt0, 0); break;
    case 1: CPU_INTC_ActivateInterrupt(USART2_IRQn, STM32F2_USART_Interrupt1, 0); break;
    case 2: CPU_INTC_ActivateInterrupt(USART3_IRQn, STM32F2_USART_Interrupt2, 0); break;
    case 3: CPU_INTC_ActivateInterrupt(UART4_IRQn, STM32F2_USART_Interrupt3, 0); break;
    case 4: CPU_INTC_ActivateInterrupt(UART5_IRQn, STM32F2_USART_Interrupt4, 0); break;
    case 5: CPU_INTC_ActivateInterrupt(USART6_IRQn, STM32F2_USART_Interrupt5, 0);
    }
...
BOOL CPU_USART_Uninitialize( int ComPortNum )
{
    ...
    g_STM32F2_Uart_Ports[ComPortNum]->CR1 = 0; // stop uart
...
{
    if (g_STM32F2_Uart_Ports[ComPortNum]->SR & USART_SR_TXE) return TRUE;
    return FALSE;
}

BOOL CPU_USART_TxShiftRegisterEmpty( int ComPortNum )
{
    if (g_STM32F2_Uart_Ports[ComPortNum]->SR & USART_SR_TC) return TRUE;
    return FALSE;
}
...
#endif
    g_STM32F2_Uart_Ports[ComPortNum]->DR = c;
}
...
{
    if (g_STM32F2_Uart_Ports[ComPortNum]->CR1 & USART_CR1_TXEIE) return TRUE;
    return FALSE;
}
...
{
    if (g_STM32F2_Uart_Ports[ComPortNum]->CR1 & USART_CR1_RXNEIE) return TRUE;
    return FALSE;
}
...
BOOL CPU_USART_TxHandshakeEnabledState( int ComPortNum )
{
    ...
    if (g_STM32F2_Uart_Ports[ComPortNum]->CR3 & USART_CR3_CTSE)
    {
        GPIO_PIN pin = 0x0B;
...
void CPU_USART_GetPins( int ComPortNum, GPIO_PIN& rxPin, GPIO_PIN& txPin,GPIO_PIN& ctsPin, GPIO_PIN& rtsPin )
{
    ...
    if ((UINT32)ComPortNum >= ARRAYSIZE_CONST_EXPR(g_STM32F2_Uart_RxD_Pins)) return;
    rxPin = g_STM32F2_Uart_RxD_Pins[ComPortNum];
    txPin = g_STM32F2_Uart_TxD_Pins[ComPortNum];
    if ((UINT32)ComPortNum >= ARRAYSIZE_CONST_EXPR(g_STM32F2_Uart_CTS_Pins)) return; // no CTS/RTS
    ctsPin = g_STM32F2_Uart_CTS_Pins[ComPortNum];
    rtsPin = g_STM32F2_Uart_RTS_Pins[ComPortNum];
}
