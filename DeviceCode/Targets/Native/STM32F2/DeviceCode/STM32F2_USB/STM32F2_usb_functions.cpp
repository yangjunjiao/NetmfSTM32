...
//  Implementation for STM32F2: Copyright (c) Oberon microsystems, Inc.
//
...
#if !defined(STM32F2_USB_FS_USE_ID_PIN)
    #define STM32F2_USB_FS_USE_ID_PIN 0
#endif
#if !defined(STM32F2_USB_FS_USE_VB_PIN)
    #define STM32F2_USB_FS_USE_VB_PIN 0
#endif
#if !defined(STM32F2_USB_HS_USE_ID_PIN)
    #define STM32F2_USB_HS_USE_ID_PIN 0
#endif
#if !defined(STM32F2_USB_HS_USE_VB_PIN)
    #define STM32F2_USB_HS_USE_VB_PIN 0
#endif


#define MAX_EP0_SIZE 8      // control channel packet size
#define STM32F2_Num_EP_FS 4 // OTG FS supports 4 endpoints
#define STM32F2_Num_EP_HS 6 // OTG FS supports 6 endpoints
    ...
    #define STM32F2_USB_REGS(c) (c ? OTG_HS : OTG_FS)
    
    #define STM32F2_USB_IS_HS(c) c
    #define STM32F2_USB_IDX(c) c
    #define STM32F2_USB_FS_IDX 0
    #define STM32F2_USB_HS_IDX 1
    
    #define STM32F2_USB_DM_PIN(c) (c ? 30 : 11) // B14,A11
    #define STM32F2_USB_DP_PIN(c) (c ? 31 : 12) // B15,A12
    #define STM32F2_USB_VB_PIN(c) (c ? 29 :  9) // B13,A9
    #define STM32F2_USB_ID_PIN(c) (c ? 28 : 10) // B12,A10
    
    #define STM32F2_USB_USE_ID_PIN(c) (c ? STM32F2_USB_HS_USE_ID_PIN : STM32F2_USB_FS_USE_ID_PIN)
    #define STM32F2_USB_USE_VB_PIN(c) (c ? STM32F2_USB_HS_USE_VB_PIN : STM32F2_USB_FS_USE_VB_PIN)
    #define STM32F2_USB_ALT_MODE(c) (GPIO_ALT_MODE)(c ? 0xC2 : 0xA2) // AF12,AF10
    
    #if USB_MAX_QUEUES <= STM32F2_Num_EP_FS
        #define USB_MAX_EP(c) USB_MAX_QUEUES
        #define USB_MAX_BUFFERS (2 * USB_MAX_QUEUES - 2)
    #elif USB_MAX_QUEUES <= STM32F2_Num_EP_HS
        #define USB_MAX_EP(c) (c ? USB_MAX_QUEUES : STM32F2_Num_EP_FS)
        #define USB_MAX_BUFFERS (USB_MAX_QUEUES + STM32F2_Num_EP_FS - 2)
    #else
        #define USB_MAX_EP(c) (c ? STM32F2_Num_EP_HS : STM32F2_Num_EP_FS)
        #define USB_MAX_BUFFERS (STM32F2_Num_EP_HS + STM32F2_Num_EP_FS - 2)
    #endif
    

#elif defined(STM32F2_USB_HS)
    // use USB OTG High Speed
    
    #define STM32F2_USB_REGS(c) OTG_HS
    
    #define STM32F2_USB_IS_HS(c) TRUE
    #define STM32F2_USB_IDX(c) 0
    #define STM32F2_USB_FS_IDX 0
    #define STM32F2_USB_HS_IDX 0

    #define STM32F2_USB_DM_PIN(c) 30 // B14
    #define STM32F2_USB_DP_PIN(c) 31 // B15
    #define STM32F2_USB_VB_PIN(c) 29 // B13
    #define STM32F2_USB_ID_PIN(c) 28 // B12
    
    #define STM32F2_USB_USE_ID_PIN(c) STM32F2_USB_HS_USE_ID_PIN
    #define STM32F2_USB_USE_VB_PIN(c) STM32F2_USB_HS_USE_VB_PIN
    #define STM32F2_USB_ALT_MODE(c) (GPIO_ALT_MODE)0xC2; // AF12

    #if USB_MAX_QUEUES <= STM32F2_Num_EP_HS
        #define USB_MAX_EP(c) USB_MAX_QUEUES
        #define USB_MAX_BUFFERS (USB_MAX_QUEUES - 1)
    #else
        #define USB_MAX_EP(c) STM32F2_Num_EP_HS
        #define USB_MAX_BUFFERS (STM32F2_Num_EP_HS - 1)
    #endif
    ...
    #define STM32F2_USB_REGS(c) OTG_FS
    
    #define STM32F2_USB_IS_HS(c) FALSE
    #define STM32F2_USB_IDX(c) 0
    #define STM32F2_USB_FS_IDX 0
    #define STM32F2_USB_HS_IDX 0
    
    #define STM32F2_USB_DM_PIN(c) 11 // A11
    #define STM32F2_USB_DP_PIN(c) 12 // A12
    #define STM32F2_USB_VB_PIN(c)  9 // A9
    #define STM32F2_USB_ID_PIN(c) 10 // A10
    
    #define STM32F2_USB_USE_ID_PIN(c) STM32F2_USB_FS_USE_ID_PIN
    #define STM32F2_USB_USE_VB_PIN(c) STM32F2_USB_FS_USE_VB_PIN
    #define STM32F2_USB_ALT_MODE(c) (GPIO_ALT_MODE)0xA2; // AF10
    
    #if USB_MAX_QUEUES <= STM32F2_Num_EP_FS
        #define USB_MAX_EP(c) USB_MAX_QUEUES
        #define USB_MAX_BUFFERS (USB_MAX_QUEUES - 1)
    #else
        #define USB_MAX_EP(c) STM32F2_Num_EP_FS
        #define USB_MAX_BUFFERS (STM32F2_Num_EP_FS - 1)
    #endif
...
#define STM32F2_USB_TRDT ((4 * 48000000 - 1) / SYSTEM_CYCLE_CLOCK_HZ + 2)
...
static STM32F2_USB_STATE STM32F2_USB_ControllerState[TOTAL_USB_CONTROLLER];
 ...
 */
void STM32F2_USB_Driver_SuspendEvent (OTG_TypeDef* OTG, USB_CONTROLLER_STATE* State)
{
    ...
    ((STM32F2_USB_STATE*)State)->previousDeviceState = State->DeviceState;
 ...
 */
void STM32F2_USB_Driver_ResumeEvent (OTG_TypeDef* OTG, USB_CONTROLLER_STATE* State)
{
    USB_Debug("R");
    
    OTG->DCTL &= ~OTG_DCTL_RWUSIG; // remove remote wakeup signaling
    
    State->DeviceState = ((STM32F2_USB_STATE*)State)->previousDeviceState;
    USB_StateCallback( State );
}
 ...
 */
void STM32F2_USB_Driver_ResetEvent (OTG_TypeDef* OTG, USB_CONTROLLER_STATE* State)
{
    USB_Debug("!");
    ...
    OTG->DIEP[0].CTL = OTG_DIEPCTL_USBAEP | 3; // Tx FIFO 0, 8 bytes packet size
    OTG->DOEP[0].CTL = OTG_DOEPCTL_USBAEP | 3;
    OTG->DIEP[0].TSIZ = 0;
    ...
    UINT32 eptype = ((STM32F2_USB_STATE*)State)->EP_Type >> 2; // endpoint types (2 bits / endpoint)
    UINT32 i = 1, bit = 2;
    while (eptype) {
 ...
 */
void STM32F2_USB_Driver_EP_RX_Int (OTG_TypeDef* OTG, USB_CONTROLLER_STATE* State,
                                   UINT32 ep, UINT32 count)
{
    ...
    if (ep == 0) { // control endpoint
        ...
        pd = (UINT32*)((STM32F2_USB_STATE*)State)->ep0Buffer;
        State->Data = (BYTE*)pd;
 ...
 */
void STM32F2_USB_Driver_EP_In_Int (OTG_TypeDef* OTG, USB_CONTROLLER_STATE* State, UINT32 ep)
{
    ASSERT_IRQ_MUST_BE_OFF();
 ...
 */
void STM32F2_USB_Driver_Handle_Setup (OTG_TypeDef* OTG, USB_CONTROLLER_STATE* State)
{
    ...
    STM32F2_USB_Driver_EP_In_Int(OTG, State, 0);
    ...
    if (result == USB_STATE_CONFIGURATION) {
        for (int ep = 1; ep < State->EndpointCount; ep++) {
            if( State->Queues[ep] && State->IsTxQueue[ep] ) {
                STM32F2_USB_Driver_EP_In_Int(OTG, State, ep);
            }
        }
    }
}
 ...
 */
void STM32F2_USB_Driver_EP_Out_Int (OTG_TypeDef* OTG, USB_CONTROLLER_STATE* State, UINT32 ep)
{
    ASSERT_IRQ_MUST_BE_OFF();
    ...
    if (ep == 0) { // control endpoint
        USB_Debug("$");
        // enable endpoint
        OTG->DOEP[0].TSIZ = OTG_DOEPTSIZ_STUPCNT | OTG_DOEPTSIZ_PKTCNT_1 | MAX_EP0_SIZE;
        OTG->DOEP[0].CTL |= OTG_DOEPCTL_EPENA | OTG_DOEPCTL_CNAK;
        // Handle Setup data in upper layer
        STM32F2_USB_Driver_Handle_Setup(OTG, State);
 ...
 */
void STM32F2_USB_Driver_Interrupt (OTG_TypeDef* OTG, USB_CONTROLLER_STATE* State)
{
    INTERRUPT_START;
    ...
    while (intPend & OTG_GINTSTS_RXFLVL) { // RxFifo non empty
        ...
        if (status == OTG_GRXSTSP_PKTSTS_PR    // data received
         || status == OTG_GRXSTSP_PKTSTS_SR) { // setup received
            STM32F2_USB_Driver_EP_RX_Int(OTG, State, ep, count);
        } else {
    ...
    if (intPend & OTG_GINTSTS_IEPINT) { // IN endpoint
        ...
        int ep = 0;
        while (bits) {
            if (bits & 1) STM32F2_USB_Driver_EP_In_Int(OTG, State, ep);
            ep++;
            bits >>= 1;
        }         
    }
    ...
    if (intPend & OTG_GINTSTS_OEPINT) { // OUT endpoint
        ...
        int ep = 0;
        while (bits) {
            if (bits & 1) STM32F2_USB_Driver_EP_Out_Int(OTG, State, ep);
            ep++;
            bits >>= 1;
        } 
    }
 ...
 */
void STM32F2_USB_Driver_FS_Interrupt (void* param)
{
    STM32F2_USB_Driver_Interrupt(OTG_FS, &STM32F2_USB_ControllerState[STM32F2_USB_FS_IDX].state);
}

/*
 * OTG HS Interrupt Handler
 */
void STM32F2_USB_Driver_HS_Interrupt (void* param)
{
    STM32F2_USB_Driver_Interrupt(OTG_HS, &STM32F2_USB_ControllerState[STM32F2_USB_HS_IDX].state);
}


USB_CONTROLLER_STATE * CPU_USB_GetState( int Controller )
{
    if ((UINT32)Controller >= TOTAL_USB_CONTROLLER) return NULL;
    return &STM32F2_USB_ControllerState[STM32F2_USB_IDX(Controller)].state;
}

HRESULT CPU_USB_Initialize( int Controller )
{
    if ((UINT32)Controller >= TOTAL_USB_CONTROLLER) return S_FALSE;
    
    // enable USB clock
    if (STM32F2_USB_IS_HS(Controller)) { // HS on AHB1
        RCC->AHB1ENR |= RCC_AHB1ENR_OTGHSEN;
    } else { // FS on AHB2
        RCC->AHB2ENR |= RCC_AHB2ENR_OTGFSEN;
    }
    
    USB_CONTROLLER_STATE *State = &STM32F2_USB_ControllerState[STM32F2_USB_IDX(Controller)].state;
    
    OTG_TypeDef* OTG = STM32F2_USB_REGS(Controller);
    ...
    OTG->GUSBCFG = OTG_GUSBCFG_FDMOD        // force device mode
                 | STM32F2_USB_TRDT << 10   // turnaround time
    ...
    if (STM32F2_USB_USE_VB_PIN(Controller) == 0) { // no Vbus pin
        OTG->GCCFG |= OTG_GCCFG_NOVBUSSENS; // disable vbus sense
    }
    
    // setup usb state variables
    ((STM32F2_USB_STATE*)State)->pinsProtected = TRUE;
    State->EndpointStatus = ((STM32F2_USB_STATE*)State)->endpointStatus;
    ...
    UINT32 queueIdx = STM32F2_USB_IDX(Controller) ? USB_MAX_EP(0) - 1 : 0; // first queue buffer
    ...
    if (epType == 0) return S_FALSE;

    ((STM32F2_USB_STATE*)State)->EP_Type = epType;

    HAL_Time_Sleep_MicroSeconds(1000); // asure host recognizes reattach

    // setup hardware
    CPU_USB_ProtectPins(Controller, FALSE);
    if (STM32F2_USB_IS_HS(Controller)) { // HS
        CPU_INTC_ActivateInterrupt(OTG_HS_IRQn,         STM32F2_USB_Driver_HS_Interrupt, 0);
        CPU_INTC_ActivateInterrupt(OTG_HS_WKUP_IRQn,    STM32F2_USB_Driver_HS_Interrupt, 0);
    } else { // FS
        CPU_INTC_ActivateInterrupt(OTG_FS_IRQn,         STM32F2_USB_Driver_FS_Interrupt, 0);
        CPU_INTC_ActivateInterrupt(OTG_FS_WKUP_IRQn,    STM32F2_USB_Driver_FS_Interrupt, 0);
    }
...
BOOL CPU_USB_StartOutput( USB_CONTROLLER_STATE* State, int ep )
{
    ...
    OTG_TypeDef* OTG = STM32F2_USB_REGS(State->ControllerNum);
    ...
    if (irq.WasDisabled()) { // check all endpoints for pending actions
        STM32F2_USB_Driver_Interrupt(OTG, State);
    }
...
BOOL CPU_USB_ProtectPins( int Controller, BOOL On )
{
    ...
    if (((STM32F2_USB_STATE*)State)->EP_Type == 0) return FALSE;  // not yet initialized
    ...
    if (On) {
        if (!((STM32F2_USB_STATE*)State)->pinsProtected) {
        ...
        if (((STM32F2_USB_STATE*)State)->pinsProtected) {
            ...
            ((STM32F2_USB_STATE*)State)->pinsProtected = FALSE;

            GPIO_ALT_MODE altMode = STM32F2_USB_ALT_MODE(Controller);
            CPU_GPIO_DisablePin(STM32F2_USB_DM_PIN(Controller), RESISTOR_DISABLED, 0, altMode);
            CPU_GPIO_DisablePin(STM32F2_USB_DP_PIN(Controller), RESISTOR_DISABLED, 0, altMode);
            if (STM32F2_USB_USE_ID_PIN(Controller)) {
                CPU_GPIO_DisablePin(STM32F2_USB_ID_PIN(Controller), RESISTOR_DISABLED, 0, altMode);
            }
