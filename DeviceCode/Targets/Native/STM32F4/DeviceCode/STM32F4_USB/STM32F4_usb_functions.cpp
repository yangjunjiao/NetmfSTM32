...
#define STM32F4_Num_EP_HS 6 // OTG FS supports 6 endpoints
    ...
    #define STM32F4_USB_ALT_MODE(c) (GPIO_ALT_MODE)(c ? 0xC2 : 0xA2) // AF12,AF10
    ...
    #define STM32F4_USB_ALT_MODE(c) (GPIO_ALT_MODE)0xC2; // AF12
    ...
    #define STM32F4_USB_ALT_MODE(c) (GPIO_ALT_MODE)0xA2; // AF10
 ...
 */
void STM32F4_USB_Driver_EP_RX_Int (OTG_TypeDef* OTG, USB_CONTROLLER_STATE* State,
...
void STM32F4_USB_Driver_Interrupt (OTG_TypeDef* OTG, USB_CONTROLLER_STATE* State)
{
    ...
    if (intPend & OTG_GINTSTS_IEPINT) { // IN endpoint
        ...
        int ep = 0;
        while (bits) {
            if (bits & 1) STM32F4_USB_Driver_EP_In_Int(OTG, State, ep);
            ep++;
            bits >>= 1;
        }         
    }
    ...
    if (intPend & OTG_GINTSTS_OEPINT) { // OUT endpoint
        ...
        int ep = 0;
        while (bits) {
            if (bits & 1) STM32F4_USB_Driver_EP_Out_Int(OTG, State, ep);
            ep++;
            bits >>= 1;
        } 
    }
...
HRESULT CPU_USB_Initialize( int Controller )
{
    ...
    // enable USB clock
    if (STM32F4_USB_IS_HS(Controller)) { // HS on AHB1
    ...
    if (STM32F4_USB_IS_HS(Controller)) { // HS
...
BOOL CPU_USB_ProtectPins( int Controller, BOOL On )
{
    ...
    if (((STM32F4_USB_STATE*)State)->EP_Type == 0) return FALSE;  // not yet initialized
