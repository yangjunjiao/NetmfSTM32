...
void SOCKETS_CreateTcpIpProcessor(HAL_CALLBACK_FPN callback, void* arg)
{
    ...
    Sockets_LWIP_Driver::CreateTcpIpProcessor(callback, arg);
}

void SOCKETS_RestartTcpIpProcessor(UINT32 timeFromNow_us)
{
    NATIVE_PROFILE_PAL_COM();
    Sockets_LWIP_Driver::RestartTcpIpProcessor(timeFromNow_us);
}
...
{
    // call back lwIP
    g_Sockets_LWIP_Driver.m_TCP_IP_state_machine_callback(arg);
...
{
    // remember where to call back into lwIP
...
void Sockets_LWIP_Driver::RestartTcpIpProcessor(UINT32 timeFromNow_us)
{
    ...
    {
        g_Sockets_LWIP_Driver.m_TCP_IP_state_machine.EnqueueDelta(timeFromNow_us);
    }
}
...
void Sockets_LWIP_Driver::UnregisterSocket( SOCK_SOCKET sock )
{
    ...
    g_Sockets_LWIP_Driver.m_socketHandles[g_Sockets_LWIP_Driver.m_cntSockets].m_flags  = 0;
    g_Sockets_LWIP_Driver.m_socketHandles[g_Sockets_LWIP_Driver.m_cntSockets].m_socket = NULL;
}
