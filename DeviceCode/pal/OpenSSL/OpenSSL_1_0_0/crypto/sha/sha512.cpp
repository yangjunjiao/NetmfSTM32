...
int SHA512_Final (unsigned char *md, SHA512_CTX *c)
	...
	n++;
	if (n > (sizeof(c->u)-16))
		TINYCLR_SSL_MEMSET (p+n,0,sizeof(c->u)-n), n=0,
		sha512_block_data_order (c,p,1);

	TINYCLR_SSL_MEMSET (p+n,0,sizeof(c->u)-16-n);
#ifdef	B_ENDIAN
...
int SHA512_Update (SHA512_CTX *c, const void *_data, size_t len)
	...
	if (c->num != 0)
		...
		if (len < n)
			{
			TINYCLR_SSL_MEMCPY (p+c->num,data,len), c->num += (unsigned int)len;
			return 1;
			}
		else	{
			TINYCLR_SSL_MEMCPY (p+c->num,data,n), c->num = 0;
			len-=n, data+=n;
			sha512_block_data_order (c,p,1);
			}
		}
	...
	if (len >= sizeof(c->u))
		...
		if ((size_t)data%sizeof(c->u.d[0]) != 0)
			while (len >= sizeof(c->u))
				TINYCLR_SSL_MEMCPY (p,data,sizeof(c->u)),
				sha512_block_data_order (c,p,1),
				len  -= sizeof(c->u),
				data += sizeof(c->u);
		else
	...
	if (len != 0)	TINYCLR_SSL_MEMCPY (p,data,len), c->num = (int)len;
