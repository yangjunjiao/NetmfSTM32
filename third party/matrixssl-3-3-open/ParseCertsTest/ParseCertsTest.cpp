// ParseCertsTest.cpp : Definiert den Einstiegspunkt für die Konsolenanwendung.
//

#include "stdafx.h"
#include "parseCerts.h"
#include <matrixssl/matrixsslApi.h>

#define USE_SAMPLECERTS
#ifdef USE_SAMPLECERTS

unsigned int serverCert_size=958;
unsigned char serverCert[] = {

};

unsigned char serverCert2[] = {

};

unsigned int serverKey_size=824;
unsigned char serverKey[] = {
 
};
unsigned char serverKey2[] = {
 
};


unsigned char netMfServerCert[] = { ,
 };

 static unsigned char desktop[] = {
 ,
        };

#endif


int testSepBase64()
{
    sslKeys_t *g_pSslKeys;
       unsigned int result = 0;
            unsigned char* outCert = NULL;
            unsigned long outCertSize;
            unsigned char* outKey = NULL;
            unsigned long outKeySize;


            int iRet;
	//
	// Initialize the SSL library.
	//
	iRet = matrixSslOpen();
	if (iRet < 0) {
		//
		// Something went wrong so kill the application with an error code.
		//
		//MATRIXSSL_PERROR("MatrixSSL library open failure.  Exiting\n");
		return (PS_FAILURE);
	}

	iRet = matrixSslNewKeys(&g_pSslKeys);

	if (iRet < 0) {
		//MATRIXSSL_PERROR("MatrixSSL library key init failure.  Exiting\n");
		matrixSslDeleteKeys(g_pSslKeys);
		matrixSslClose();
		//
		// Something went wrong so kill the application with an error code.
		//
		return (PS_FAILURE);
	}

            outCert = (unsigned char*) malloc(sizeof(serverCert2));
            outKey = (unsigned char*)malloc(sizeof(serverKey));

            result = psBase64decode(serverCert2, sizeof(serverCert2), outCert, &outCertSize);
            result = psBase64decode(serverCert, sizeof(serverCert), outCert, &outCertSize);
            result = psBase64decode(serverKey, sizeof(serverKey), outKey, &outKeySize);

            if (matrixSslLoadRsaKeysMem(g_pSslKeys, outCert, outCertSize,
						outKey, outKeySize, NULL, 0) < 0) {
				return PS_FAILURE;
			}
            return PS_SUCCESS;
}

int testParsNetMfCerts()
{
    sslKeys_t *g_pSslKeys;
       unsigned int result = 0;
            unsigned char* outCert = NULL;
            unsigned long outCertSize;
            unsigned char* outKey = NULL;
            unsigned long outKeySize;


            int iRet;
	//
	// Initialize the SSL library.
	//
	iRet = matrixSslOpen();
	if (iRet < 0) {
		//
		// Something went wrong so kill the application with an error code.
		//
		//MATRIXSSL_PERROR("MatrixSSL library open failure.  Exiting\n");
		return (PS_FAILURE);
	}

	iRet = matrixSslNewKeys(&g_pSslKeys);

	if (iRet < 0) {
		//MATRIXSSL_PERROR("MatrixSSL library key init failure.  Exiting\n");
		matrixSslDeleteKeys(g_pSslKeys);
		matrixSslClose();
		//
		// Something went wrong so kill the application with an error code.
		//
		return (PS_FAILURE);
	}

            
            parseServerCert(netMfServerCert, sizeof(netMfServerCert));
            parseClientCA(desktop, sizeof(desktop));

            return LoadRsaKeys(g_pSslKeys);
}

int _tmain(int argc, _TCHAR* argv[])
{
   int result = 0;
   //result = testSepBase64();

   if(result == PS_SUCCESS)
       printf("Test passed\n");
   else
       printf("Test failed\n");

   parseServerCert(netMfServerCert, sizeof(netMfServerCert));
   parseClientCA(desktop, sizeof(desktop));
 }

